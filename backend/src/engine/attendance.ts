...
export function computeAttendance(rng: () => number, ctx: AttendanceContext): number {
  ...
  // Rivalry week sells out buildings that don't otherwise come close —
  // scaled by how big a deal this particular rivalry has become.
  if (ctx.isRivalry) turnout += 0.1 + ((ctx.rivalryIntensity ?? 50) / 100) * 0.15;
