...
export function startConferenceTournaments(state: WorldState, seasonYear: number, division: Division, startDate: Date): void {
  ...
  for (const conf of conferences) {
    ...
    state.tournaments.push({ id: tournamentId, seasonYear, type: "CONFERENCE_TOURNAMENT", name: null, format: null, division, conferenceId: conf.id });
...
export function startNationalTournaments(state: WorldState, seasonYear: number, division: Division, startDate: Date): void {
  ...
  if (division === "D1") {
    ...
    const ncaaId = newId();
    state.tournaments.push({ id: ncaaId, seasonYear, type: "NCAA_TOURNAMENT", name: null, format: null, division, conferenceId: null });
    ...
    if (nitPoolAll.length >= 4) {
      const nitId = newId();
      state.tournaments.push({ id: nitId, seasonYear, type: "NIT", name: null, format: null, division, conferenceId: null });
    ...
    const tournamentId = newId();
    state.tournaments.push({ id: tournamentId, seasonYear, type, name: null, format: null, division, conferenceId: null });
